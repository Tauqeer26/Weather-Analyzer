...
class MatPLOTS:
    ...
    def average_annual_precipitation_by_country(self, year):
        try:
            ...
            '''
            # Executing the SQL query with the provided parameter
            ...
            # Plotting the bar chart based on the retrieved data
            plt.figure(figsize=(10, 6))
            plt.bar(countries, avg_precipitations, color='skyblue')
            ...
            print(ex)    
    def plot_city_weather_statistics(self, date_from, date_to):
        try:
            cursor =self.connection.cursor()
            #SQL Query
            query = """
    ...
    def plot_daily_temperature(self, city_id, year, month):
        try:
            # Establishing connection to the database
            cursor = self.connection.cursor()
            #SQL Query
    ...
    def average_mean_temp_and_precipitation_by_city(self,date_from, date_to):
        try:
            ...
            # Plotting
            plt.figure(figsize=(10, 6))
            plt.bar(cities, avg_temps, color='skyblue', label='Average Mean Temperature')
            plt.bar(cities, avg_precipitations, color='orange', label='Average Precipitation')
            plt.xlabel('City ID')
            plt.ylabel('Values')
